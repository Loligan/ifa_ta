...
class PageObject
{
    ...
    const FILES_PATH = __DIR__ . '/../Resources/test_files/';
    ...
    protected static function checkPathByRegex($regexPath)
    {
        $url = FeatureContext::getWebDriver()->getCurrentURL();
        if (!preg_match($regexPath, $url)) {
            throw new\Exception('We not on page by regex "' . $regexPath . '". You locate on url: ' . $url);
        }
    }
